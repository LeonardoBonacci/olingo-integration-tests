...
/**
 * This processor will deliver entity collections, single entities as well as
 * properties of an entity. This is a very simple example which should give you
 * a rough guideline on how to implement such an processor. See the JavaDoc of
 * the server.api interfaces for more information.
 */
public class CarsProcessor implements EntityCollectionProcessor, EntityProcessor, PrimitiveProcessor,
	...
	public void readEntityCollection(final ODataRequest request, ODataResponse response, final UriInfo uriInfo,
		...
		// Second we fetch the data for this specific entity set from the mock database
		// and transform it into an EntitySet
		// object which is understood by our serialization
		EntityCollection entitySet = dataProvider.readAll(edmEntitySet);

		// Next we create a serializer based on the requested format. This could also be
		// a custom format but we do not
	...
	public void readEntity(final ODataRequest request, ODataResponse response, final UriInfo uriInfo,
		...
		if (entity == null) {
			...
			// If an entity was found we proceed by serializing it and sending it to the
			// client.
			ODataSerializer serializer = odata.createSerializer(requestedContentType);
			final ExpandOption expand = uriInfo.getExpandOption();
			final SelectOption select = uriInfo.getSelectOption();
	...
	public void createEntity(ODataRequest request, ODataResponse response, UriInfo uriInfo, ContentType requestFormat,
		...
		// WON'TODO: EXECUTE THE ACTUAL CREATE, INSTEAD, WE SET A DUMMY ID....
		int newId = new Random().nextInt(10000);
		Property idProperty = requestEntity.getProperty("Id");
		if (idProperty != null) {
			idProperty.setValue(ValueType.PRIMITIVE, Integer.valueOf(newId));
		} else {
			// as of OData v4 spec, the key property can be omitted from the POST request
			// body
			requestEntity.getProperties().add(new Property(null, "Id", ValueType.PRIMITIVE, newId));
		}
		requestEntity.setId(DataProvider.createId(/*only works for*/ "Manufacturers", newId)); 
	...
	public void deleteEntity(ODataRequest request, ODataResponse response, UriInfo uriInfo)
		...
		// WON'TODO: EXECUTE THE ACTUAL DELETE....
	...
	public void readPrimitiveValue(ODataRequest request, ODataResponse response, UriInfo uriInfo, ContentType format)
		...
		if (entity == null) {
			...
			// Next we get the property value from the entity and pass the value to
			// serialization
	...
	private EdmEntitySet getEdmEntitySet(final UriInfoResource uriInfo) throws ODataApplicationException {
		...
		/*
		 * Here we should interpret the whole URI but in this example we do not support
		 * navigation so we throw an exception
		 */
